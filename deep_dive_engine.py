...
def analyze(ticker, get_price_history, get_ticker_info, get_cashflow_df,
    ...
    # Rolling mean runs over _close_series (the de-NaN'd close history), NOT
    # the raw window_3mo["Close"] - confirmed via a live Railway diagnostic
    # (2026-08-29) that yfinance routinely returns the most recent trading
    # day's Close as NaN (that day's bar not yet finalized upstream when
    # the 30-min-cached history pull ran) on ordinary, highly liquid names
    # (reproduced live on CPRT, AAPL, and MSFT simultaneously - one NaN row
    # each, always the newest). rolling(50) requires 50 non-null values in
    # its window by default; with a raw trailing NaN, the last window is
    # one short (49 valid) and the whole calculation silently comes back
    # NaN, wrongly triggering the ma50_defaulted fallback below on totally
    # ordinary stocks with abundant real history. Using the same
    # already-cleaned _close_series that current_price/fear_score use
    # (same fix already applied for current_price after an earlier NaN-row
    # sighting on NYSE:NU - see app.py's matching comment) removes that
    # single bad row from the window instead of leaving a gap in it, so a
    # real 50-day average still comes back whenever 50 real closes exist.
    ma50_raw = _close_series.rolling(50).mean().iloc[-1]
