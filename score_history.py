...
def _conn():
    ...
    #
    # Commit J (21 Sep 2026): trading_status ('stale' or NULL) - see
    # nightly_scan.analyze_ticker_lite()'s own "Trading Status" field
...
def last_real_price_date(ticker):
    """Commit J (21 Sep 2026): for a ticker currently flagged "stale"
    (nightly_scan.analyze_ticker_lite()'s ghost-price guard - see that
    function's own comment), the day its price was last GENUINELY
    different from the day before it - i.e. the last day a real trade
    was actually captured, not just the most recent day it happened to
    be scanned (which, for a ghost-priced ticker, is every night,
    always showing the same frozen number).

    Walks the stored series backward from the most recent row and
    returns the day at the START of the trailing run of identical
    prices - the day the price last changed FROM is the last day it
    was real; every day after that just repeats it. None if there's no
    history at all, or the single most recent row has no price to
    compare.

    This is deliberately NOT computed at scan time and stored as its
    own column - it depends on the FULL trailing run's length, which
    only the accumulated series can answer, and it needs no daily
    write of its own: the banner that reads this (snapshot_render.py)
    only ever needs it for a ticker score_history.latest() already
    reports as flagged."""
    rows = series(ticker, limit_days=730)
    if not rows:
        return None
    last_price = rows[-1].get("price")
    if last_price is None:
        return None
    idx = len(rows) - 1
    while idx > 0 and rows[idx - 1].get("price") == last_price:
        idx -= 1
    return rows[idx].get("day")
