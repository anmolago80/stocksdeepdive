...
EN = {
    ...
    # per the task's own instruction, in both places rather than
    # picking one.
    "top100.subtitle": (
        "Qualitative scores and inversion scenarios are generated by "
        "Claude ({model}, {date}) - subjective analytical judgments shown "
    ...
    "top100.methodology_body": (
        ...
        "accounting quality, balance sheet & fixed charges, management "
        "quality, capital allocation, and reinvestment runway - each an "
        "integer 1-5 (5 is always the good outcome for a holder) plus a "
        "one-line justification naming its source period. A dimension the "
        "model has no confident public-record knowledge of is left blank "
        "(null), never guessed at a middle value; three or more blanks and "
        "the company is marked NOT RATED rather than scored. Claude also "
        "writes a one-sentence \"inversion\" - the single most plausible "
        "scenario that could seriously damage the company, plus a severity "
    ...
    "top100.methodology_decircularisation": (
        ...
        "numeric Quality factor straight from public financial data - "
        "return on equity, net profit margin, return on invested capital, "
        "revenue growth, earnings growth, free cash flow, and the "
        "debt-to-equity ratio. Claude is explicitly instructed to add "
        "judgment that numeric pipeline cannot see, never to restate what "
        "it already measures: competitive position never re-scores "
        "absolute profitability (only the durability of an edge versus "
        "named peers); pricing power never re-scores today's margin "
        "(only the forward ability to defend it); balance sheet & fixed "
        "charges never re-scores the raw debt-to-equity ratio (only the "
        "maturity wall, coverage and fixed-cost rigidity); capital "
        "allocation never re-scores the current return-on-capital level "
        "(only incremental deployment decisions); reinvestment runway "
    ...
    # market_structure_onefoot_mock.html") - display-only, same
    # "carried through, never scored" status as every other Top 100
    # display field.
    "top100.market_structure_label": "Market structure",
    "top100.easy_decision_label": "Easy decision?",
    "top100.structure_monopoly": "MONOPOLY",
    "top100.structure_duopoly": "DUOPOLY",
    "top100.structure_oligopoly": "OLIGOPOLY",
    ...
    "top100.sentiment_neutral": "Neutral",
    "top100.sentiment_greedy": "Greedy",
    "top100.inversion_line": "⚠ Inversion: {scenario} — severity {severity}/5",
    "top100.refresh_all_button": "🔄 Refresh all (owner)",
    "top100.refresh_all_running": "Submitting a fresh Batch API scoring run for every pooled company...",
    "top100.refresh_all_submitted": (
        "Refresh submitted - batch {batch_id} is now scoring every pooled "
        "company under the current rubric."
    ),
    "top100.refresh_all_nothing": "Nothing to refresh - the pool is empty (no scan data yet).",
    ...
    # Hover-tooltip anchor text (task's own "the score's anchor
    # meaning") - concise restatements of top100_engine._SYSTEM_
    # PROMPT's own full anchors, one line each, for the chip title=
    # attribute and the tap-expand detail.
    "top100.dim_anchor_ai_exposure": "5 = immune to or a beneficiary of AI; 1 = directly substitutable by an AI tool",
    "top100.dim_anchor_competitive_position": "5 = structurally more profitable than named peers for a durable reason; 1 = undifferentiated among stronger rivals",
    "top100.dim_anchor_regulatory_legal": "5 = regulation compels purchase of the product; 1 = existential political/procurement risk or major litigation overhang",
    "top100.dim_anchor_customer_concentration": "5 = thousands of small customers; 1 = one customer is more than ~30% of revenue",
    "top100.dim_anchor_pricing_power": "5 = repeatedly raises prices above inflation without losing volume; 1 = a pure price-taker",
    "top100.dim_anchor_accounting_quality": "5 = cash conversion 80-120% of net income with minimal cost capitalisation; 1 = aggressive capitalisation materially inflates reported FCF",
    "top100.dim_anchor_balance_sheet_fixed_charges": "5 = net cash with a flexible cost base; 1 = leveraged with a near-term debt wall or heavy fixed costs",
...
ES = {
    ...
    "top100.methodology_body": (
        ...
        "de precios y traspaso de costos, calidad contable, balance y "
        "cargos fijos, calidad de la gestión, asignación de capital y "
        "margen de reinversión - cada una un número entero de 1 a 5 (5 "
        "siempre es el resultado bueno para quien mantiene la acción) más "
        "una justificación de una línea que indica su período de "
        "referencia. Una dimensión sobre la que el modelo no tiene "
        "conocimiento confiable y público se deja en blanco (nulo), nunca "
        "se adivina con un valor medio; tres o más dimensiones en blanco y "
        "la empresa se marca como NO EVALUADA en lugar de puntuarse. "
        "Claude también escribe una \"inversión\" de una línea - el "
        "escenario individual más plausible que podría dañar seriamente a "
    ...
    "top100.methodology_decircularisation": (
        ...
        "factor de Calidad puramente numérico directamente a partir de "
        "datos financieros públicos - retorno sobre el patrimonio, margen "
        "de beneficio neto, retorno sobre el capital invertido, "
        "crecimiento de ingresos, crecimiento de beneficios, flujo de "
        "caja libre y el ratio deuda-patrimonio. A Claude se le instruye "
        "explícitamente para aportar un juicio que esa canalización "
        "numérica no puede ver, nunca para repetir lo que ya mide: "
        "posición competitiva nunca vuelve a puntuar la rentabilidad "
        "absoluta (solo la durabilidad de una ventaja frente a "
        "competidores nombrados); poder de fijación de precios nunca "
        "vuelve a puntuar el margen actual (solo la capacidad futura de "
        "defenderlo); balance y cargos fijos nunca vuelve a puntuar el "
        "ratio deuda-patrimonio en bruto (solo el muro de vencimientos, "
        "la cobertura y la rigidez de costos fijos); asignación de "
        "capital nunca vuelve a puntuar el nivel actual de retorno sobre "
        "el capital (solo las decisiones de despliegue incremental); "
    ...
    "top100.market_structure_label": "Estructura del mercado",
    "top100.easy_decision_label": "¿Decisión fácil?",
    "top100.structure_monopoly": "MONOPOLIO",
    "top100.structure_duopoly": "DUOPOLIO",
    "top100.structure_oligopoly": "OLIGOPOLIO",
    ...
    "top100.sentiment_neutral": "Neutral",
    "top100.sentiment_greedy": "Codicioso",
    "top100.inversion_line": "⚠ Inversión: {scenario} — severidad {severity}/5",
    "top100.refresh_all_button": "🔄 Actualizar todo (propietario)",
    "top100.refresh_all_running": "Enviando una nueva ejecución de puntuación por API por lotes para cada empresa del conjunto...",
    "top100.refresh_all_submitted": (
        "Actualización enviada - el lote {batch_id} está evaluando ahora "
        "cada empresa del conjunto bajo la rúbrica actual."
    ),
    "top100.refresh_all_nothing": "Nada que actualizar - el conjunto está vacío (aún no hay datos de escaneo).",
    ...
    "top100.dim_anchor_ai_exposure": "5 = inmune o beneficiaria de la IA; 1 = directamente sustituible por una herramienta de IA",
    "top100.dim_anchor_competitive_position": "5 = estructuralmente más rentable que competidores nombrados por una razón duradera; 1 = indiferenciada entre rivales más fuertes",
    "top100.dim_anchor_regulatory_legal": "5 = la regulación obliga a comprar el producto; 1 = riesgo político/de contratación existencial o litigio importante",
    "top100.dim_anchor_customer_concentration": "5 = miles de clientes pequeños; 1 = un cliente representa más del ~30% de los ingresos",
    "top100.dim_anchor_pricing_power": "5 = sube precios repetidamente por encima de la inflación sin perder volumen; 1 = una pura tomadora de precios",
    "top100.dim_anchor_accounting_quality": "5 = conversión de caja del 80-120% del beneficio neto con capitalización mínima de costos; 1 = capitalización agresiva infla materialmente el FCF reportado",
    "top100.dim_anchor_balance_sheet_fixed_charges": "5 = caja neta con una estructura de costos flexible; 1 = apalancada con un muro de deuda cercano o costos fijos elevados",
