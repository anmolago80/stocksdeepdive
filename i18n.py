...
EN = {
    ...
    "top100.subtitle": (
        "Qualitative scores generated by Claude ({model}, {date}) - "
    ...
    "top100.methodology_body": (
        ...
        "**AI scoring.** Each company is scored by Claude on ten qualitative "
        "dimensions - moat, pricing power, inflation exposure, AI exposure, "
        "regulatory risk, customer concentration, accounting quality, capital "
        "allocation, reinvestment runway, and balance-sheet resilience - each "
        "an integer 1-5 (5 is always the good outcome for a holder) plus a "
        ...
        "the company is marked NOT RATED rather than scored.\n\n"
        "**Composite.** Computed in plain arithmetic, never by the model: "
        "18% of the composite is this site's own margin of safety (intrinsic "
        "value vs price), rescaled across the current 100 companies so it "
        "sits on the same 0-100 scale as everything else - not the Value "
        "Score, which would double-count quality already priced into the "
        "selection. The remaining 82% is the ten AI-scored dimensions, "
        "weighted as below.\n\n"
        "**Cache and cadence.** A score is cached per ticker, calendar "
        "quarter and model - only newly-entered, unscored companies are sent "
        "to the API on an ordinary night (capped at 120 a night), with a "
        "full re-score every quarter or on demand via the owner's own "
        "\"refresh all\" button. Scoring runs through Anthropic's Batch API."
    ),
    "top100.methodology_weights_heading": "Composite weights (out of 100)",
    ...
    "top100.currency_banner": (
        ...
        "and it is never part of the composite score above."
    ),
    ...
    "top100.col_mos": "MOS",
    "top100.col_composite": "Composite",
...
ES = {
    ...
    "nav.top100": "\U0001F3C6 Top 100",
    "top100.subtitle": (
        "Puntuaciones cualitativas generadas por Claude ({model}, {date}) - "
        "cribado descrito, no un consejo."
    ),
    ...
    "top100.methodology_body": (
        ...
        "dimensiones cualitativas - foso competitivo, poder de fijación de "
        "precios, exposición a la inflación, exposición a la IA, riesgo "
        "regulatorio, concentración de clientes, calidad contable, asignación "
        "de capital, margen de reinversión y resiliencia del balance - cada "
        "una un número entero de 1 a 5 (5 siempre es el resultado bueno para "
        "quien mantiene la acción) más una justificación de una línea que "
        "indica su período de referencia. Una dimensión sobre la que el "
        "modelo no tiene conocimiento confiable y público se deja en blanco "
        "(nulo), nunca se adivina con un valor medio; tres o más dimensiones "
        "en blanco y la empresa se marca como NO EVALUADA en lugar de "
        "puntuarse.\n\n"
        "**Puntuación compuesta.** Calculada con aritmética simple, nunca por "
        "el modelo: el 18% de la puntuación compuesta es el margen de "
        "seguridad propio de este sitio (valor intrínseco frente al precio), "
        "reescalado entre las 100 empresas actuales para que quede en la "
        "misma escala de 0 a 100 que el resto - no el Value Score, que "
        "contaría dos veces la calidad ya incorporada en la selección. El "
        "82% restante son las diez dimensiones evaluadas por IA, ponderadas "
        "como se indica abajo.\n\n"
        "**Caché y periodicidad.** Una puntuación se guarda en caché por "
        "acción, trimestre calendario y modelo - solo las empresas nuevas y "
        "aún sin puntuar se envían a la API en una noche normal (con un "
        "límite de 120 por noche), con una reevaluación completa cada "
        "trimestre o bajo demanda mediante el botón \"actualizar todo\" del "
        "propietario. La puntuación se ejecuta a través de la API por lotes "
        "de Anthropic."
    ),
    "top100.methodology_weights_heading": "Ponderaciones de la puntuación compuesta (sobre 100)",
    ...
    "top100.currency_banner": (
        ...
        "calidad, y nunca forma parte de la puntuación compuesta anterior."
    ),
    ...
    "top100.col_value_score": "Value Score",
    "top100.col_mos": "MOS",
    "top100.col_composite": "Compuesta",
